...
# React 19 + TypeScript + Base UI Rules
...
- **React Compiler v1.0** — auto-memoization; leave existing `useMemo`/`useCallback`/`memo` in place (removing can change compilation output); skip manual memo only in new code
...
// useEffectEvent (React 19.2) — stable ref to latest values without re-triggering effects
...
// Rules: only call from Effects/other Effect Events, never pass to children, never in deps
```

### Activity Component (React 19.2)

Preserve component state while hiding (replaces conditional rendering for tabs/navigation):
```typescript
import { Activity } from 'react';

// Instead of: {isVisible && <Sidebar />}  (destroys state)
<Activity mode={isVisible ? 'visible' : 'hidden'}>
  <Sidebar />
</Activity>
```
- `hidden`: `display: none`, cleans up effects, defers updates, preserves DOM/component state
- Use `useLayoutEffect` cleanup for `<video>`/`<audio>` when hidden
- Effects do NOT run while hidden

### Base UI (Headless Components)

**Package**: `@base-ui/react` — unstyled, accessible components from MUI
...
// Slider
<Slider.Root value={[50]} onValueChange={setValue}>
  <Slider.Control>
    <Slider.Track>
      <Slider.Indicator />
      <Slider.Thumb />
...
- All styling via className — components are unstyled
- Use Tailwind classes directly on Base UI components
- Portal components (Dialog, Menu) need Portal wrapper for proper z-index
- Compose sub-components for full accessibility
- Never style the Root — style child components instead
...
**With React Compiler v1.0** (stable): Write plain code for new components — compiler handles memoization. Keep existing `useMemo`/`useCallback` in place; use them as escape hatches when you need a memoized value as an effect dependency.
...
// Async startTransition — state updates after await need nested startTransition
startTransition(async () => {
  const result = await saveData(formData);
  startTransition(() => setResult(result));  // Required after await
});
```
...
- Never: remove existing memo when Compiler is enabled (leave in place), useDeferredValue without memo on consumer
